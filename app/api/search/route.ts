...
function geminiTimestamp(date: Date) { date.setUTCMilliseconds(0); return date.toISOString().replace(".000Z", "Z"); }
function periodStart(period: string) { const days = ({ "7d": 7, "30d": 30, "1y": 365 } as Record<string, number>)[period]; if (!days) return undefined; const date = new Date(); date.setUTCDate(date.getUTCDate() - days); return geminiTimestamp(date); }
...
async function searchGemini(input: SearchRequest): Promise<ApiResult> {
  try {
    const start = periodStart(input.period ?? "all"); const googleSearch: Record<string, unknown> = {}; if (start) googleSearch.timeRangeFilter = { startTime: start, endTime: geminiTimestamp(new Date()) };
